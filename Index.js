...
// ---------- Simplified funding (no intermediates) ----------
...
// ---------- Drain ephemeral wallets helper ----------
    ...
    try {
        if (!usePool) {
            ...
            if (STATE.useWebFunding) {
                await fundWalletsDirect(connection, masterKeypair, childWallets.map(w => w.keypair), fundAmount, chatId);
            } else {
...
async function executeWebOfActivity(chatId, connection) {
    ...
    if (!usePool) {
        await fundWalletsDirect(connection, masterKeypair, targets, 0.05, chatId);
    }
...
async function executeHolderGrowth(chatId, connection) {
    ...
    if (!usePool) {
        bot.sendMessage(chatId, `🧹 Draining holder wallets...`);
...
// ---------- Telegram UI Menus (unchanged from original, but using new promptSetting) ----------
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    // Wallet Pool Operations (unchanged except using new promptSetting)
    ...
    else if (action === 'toggle_web_funding') { STATE.useWebFunding = !STATE.useWebFunding; showRealismMenu(chatId); }
...
logger.info("Bot is online with all critical fixes (except wallet encryption).");
