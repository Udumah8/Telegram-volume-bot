...
async function executeStrategyTemplate(chatId, connection, strategyConfig) {
    ...
    try {
        ...
        for (let cycle = 0; cycle < cycles && STATE.running && !isShuttingDown; cycle++) {
            ...
            const buyResults = await BatchSwapEngine.executeBatch(
                wallets, 
                async (wallet, idx) => { 
...
// ─────────────────────────────────────────────
// 📈 Strategy: Maker Cycles (Integrated - Personality-Driven)
// ─────────────────────────────────────────────
async function executeMakerCycles(chatId, connection) {
    ...
    return executeStrategyTemplate(chatId, connection, {
        ...
        cycles: STATE.numberOfCycles, 
        interCycleDelay: STATE.intervalBetweenActions, 
        needsFunding: !STATE.useWalletPool, 
        needsDraining: !STATE.useWalletPool
    });
}

// ─────────────────────────────────────────────
// 🕸️ Strategy: Web of Activity (Integrated)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// 📐 Strategy: Chart Pattern (Integrated)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// 👥 Strategy: Holder Growth (Integrated)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// 🐋 Strategy: Whale Simulation (Integrated)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// 📊 Strategy: Volume Boost (Integrated)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// 🔥 Strategy: Trending Modes (Integrated - Full Implementation)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// 🌪️ Strategy: Jito MEV Wash (Integrated)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// 📱 Strategy: KOL Alpha Call (Integrated)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// 🎁 Strategy: Social Proof Airdrop (Integrated)
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// 🎮 Master Engine Controller (Fully Integrated)
// ─────────────────────────────────────────────
async function startEngine(chatId) {
    ...
    if (STATE.running) { 
        bot.sendMessage(chatId, `🔄 Already running! Stop first.`, { parse_mode: 'Markdown' }); 
        return; 
    }
    ...
    await withStrategyLock(STATE.strategy, async () => {
        try {
            await withRpcFallback(async (connection) => {
                ...
                if (strategy) await strategy(chatId, connection); 
                else throw new Error(`Unknown strategy: ${STATE.strategy}`);
        ...
        `━━━━━━━━━━━━━━━━━━━━━━━`,
        {
            ...
            reply_markup: {
                inline_keyboard: [
                    ...
                    [{ text: '📈 Strategies', callback_ 'strategies' }, { text: '⚙️ Settings', callback_data: 'settings' }],
                    [{ text: '💼 Wallet Pool', callback_ 'wallet_pool' }, { text: '📊 Dashboard', callback_ 'status' }],
                    [{ text: '📜 My Wallet', callback_ 'show_wallet' }, { text: '❓ Help', callback_ 'help' }]
                ]
            }
        }
    );
...
function showStrategyMenu(chatId) {
    ...
    bot.sendMessage(chatId,
        `📈 *STRATEGY SELECTION*\n━━━━━━━━━━━━━━━━━━━━━━━\nCurrent: *${s}*\n\nChoose:`,
        {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: (s === 'STANDARD' ? '✅ ' : '') + '🌐 Standard', callback_ 'strat_standard' }, { text: (s === 'MAKER' ? '✅ ' : '') + '📈 Maker', callback_data: 'strat_maker' }],
                    [{ text: (s === 'WEB_OF_ACTIVITY' ? '✅ ' : '') + '🕸️ Web', callback_data: 'strat_web' }, { text: (s === 'SPAM' ? '✅ ' : '') + '⚡ Spam', callback_ 'strat_spam' }],
                    [{ text: (s === 'PUMP_DUMP' ? '✅ ' : '') + '🚀 Pump&Dump', callback_data: 'strat_pumpdump' }, { text: (s === 'CHART_PATTERN' ? '✅ ' : '') + '📐 Chart', callback_data: 'strat_chart' }],
                    [{ text: (s === 'HOLDER_GROWTH' ? '✅ ' : '') + '👥 Holders', callback_data: 'strat_holder' }, { text: (s === 'WHALE' ? '✅ ' : '') + '🐋 Whale', callback_ 'strat_whale' }],
                    [{ text: (s === 'VOLUME_BOOST' ? '✅ ' : '') + '📊 Boost', callback_ 'strat_volume' }, { text: (s === 'TRENDING' ? '✅ ' : '') + '🔥 Trending', callback_data: 'strat_trending' }],
                    [{ text: (s === 'JITO_MEV_WASH' ? '✅ ' : '') + '🌪️ MEV Wash', callback_ 'strat_mev_wash' }, { text: (s === 'KOL_ALPHA_CALL' ? '✅ ' : '') + '📱 KOL', callback_ 'strat_kol' }],
                    [{ text: (s === 'BULL_TRAP' ? '✅ ' : '') + '🐻 Bull Trap', callback_data: 'strat_bull' }, { text: (s === 'SOCIAL_PROOF_AIRDROP' ? '✅ ' : '') + '🎁 Airdrop', callback_ 'strat_airdrop' }],
...
function showSettingsMenu(chatId) {
    bot.sendMessage(chatId,
        `⚙️ *CONFIGURATION*\n━━━━━━━━━━━━━━━━━━━━━━━\n\nCategory:`,
        {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: '📱 Basic', callback_ 'settings_basic' }, { text: '⚡ Advanced', callback_ 'settings_advanced' }],
                    [{ text: '🎯 Strategy', callback_data: 'settings_strat' }, { text: '🎭 Realism', callback_data: 'show_realism' }],
                    [{ text: '🔌 Provider', callback_ 'provider_settings' }, { text: '🛡️ Jito', callback_data: 'settings_jito' }],
                    [{ text: '🕸️ Stealth', callback_ 'stealth_settings' }, { text: '« Back', callback_data: 'back_to_main' }]
                ]
            }
        }
    );
        ...
        `🎲 *Jitter:* \`${STATE.jitterPercentage}%\``,
        {
            ...
            reply_markup: {
                inline_keyboard: [
                    ...
                    [{ text: '💰 Min Buy', callback_data: 'set_min_buy' }, { text: '💰 Max Buy', callback_ 'set_max_buy' }],
                    [{ text: '🔁 Cycles', callback_data: 'set_cycles' }, { text: '🎲 Jitter', callback_data: 'set_jitter' }],
                    [{ text: '⏱ Delay', callback_ 'set_interval' }],
        ...
        `• Sync Buys/Sells: \`${STATE.maxSimultaneousBuys}/${STATE.maxSimultaneousSells}\``,
        {
            ...
            reply_markup: {
                inline_keyboard: [
                    ...
                    [{ text: '⚡ Concurrency', callback_data: 'set_batch_concurrency' }, { text: '👥 Wallets/Cycle', callback_ 'set_wallets_per_cycle' }],
                    [{ text: '🔄 Sync Buys/Sells', callback_ 'set_sync' }],
                    [{ text: '🔙 Back', callback_data: 'settings' }]
                ]
            }
        }
...
function showStealthSettings(chatId) {
    bot.sendMessage(chatId,
        ...
        `• Max Hop Depth: \`${STATE.makerFundingChainDepth}\``,
        {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: `Web Funding ${STATE.useWebFunding ? '🔴' : '🟢'}`, callback_ 'toggle_web_funding' }],
                    [{ text: `Level: ${STATE.fundingStealthLevel === 2 ? '➡️ Direct' : '⬅️ Multi-hop'}`, callback_ 'toggle_stealth_level' }],
                    [{ text: '🔗 Max Depth', callback_data: 'set_maker_depth' }],
                    [{ text: '🔙 Back', callback_ 'settings' }]
                ]
            }
        }
    );
        ...
        `• Volume Curve: ${STATE.useVolumeCurve ? '🟢 ON' : '🔴 OFF'}`,
        {
            ...
            reply_markup: {
                inline_keyboard: [
                    ...
                    [{ text: `Delays ${STATE.humanizedDelays ? '🔴' : '🟢'}`, callback_ 'toggle_delays' }],
                    [{ text: `Poisson ${STATE.usePoissonTiming ? '🔴' : '🟢'}`, callback_ 'toggle_poisson' }],
                    [{ text: `Slippage ${STATE.variableSlippage ? '🔴' : '🟢'}`, callback_data: 'toggle_varslip' }],
                    [{ text: `Volume ${STATE.useVolumeCurve ? '🔴' : '🟢'}`, callback_ 'toggle_vol_curve' }],
                    [{ text: '🔙 Back', callback_ 'settings' }]
                ]
            }
        }
...
function showJitoSettings(chatId) {
    bot.sendMessage(chatId,
        ...
        `• Tip: \`${STATE.jitoTipAmount}\` SOL`,
        {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: `Toggle ${STATE.useJito ? '🔴' : '🟢'}`, callback_ 'set_jito' }],
                    [{ text: '💵 Set Tip', callback_ 'set_jito_tip' }],
                    [{ text: '🔙 Back', callback_data: 'settings' }]
                ]
            }
        }
...
function showProviderMenu(chatId) {
    ...
    bot.sendMessage(chatId,
        `🔌 *Swap Provider*\nCurrent: *${p}*\nDEX: \`${STATE.targetDex}\``,
        {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: (p === 'SOLANA_TRACKER' ? '✅ ' : '') + '🌐 SolanaTracker', callback_ 'prov_tracker' }],
                    [{ text: (p === 'SOLANA_TRADE' ? '✅ ' : '') + '🎯 SolanaTrade', callback_ 'prov_trade' }],
...
function showDexMenu(chatId) {
    ...
    for (let i = 0; i < dexes.length; i += 2) {
        ...
        row.push({ text: (current === val1 ? '✅ ' : '') + label1, callback_ `dex_${val1}` });
        if (i + 1 < dexes.length) {
            const [val2, label2] = dexes[i + 1];
            row.push({ text: (current === val2 ? '✅ ' : '') + label2, callback_ `dex_${val2}` });
        }
        keyboard.push(row);
    }
    keyboard.push([{ text: '🔙 Back', callback_ 'provider_settings' }]);
...
function showWalletPoolMenu(chatId) {
    ...
    bot.sendMessage(chatId,
        ...
        `${stats.total > 0 ? `\nSample: \`${stats.firstFew[0]?.slice(0,8)}...\`` : `\n⚠️ No wallets yet`}`,
        {
            ...
            reply_markup: {
                inline_keyboard: [
                    [{ text: '🔨 Generate', callback_ 'pool_generate' }],
                    [{ text: '💰 Fund All', callback_ 'pool_fund' }, { text: '🔄 Drain All', callback_ 'pool_drain' }],
                    [{ text: '📊 Scan', callback_data: 'pool_scan' }, { text: `${STATE.useWalletPool ? '🔴 Disable' : '🟢 Enable'}`, callback_ 'pool_toggle' }],
                    [{ text: '⚡ Concurrency', callback_data: 'set_batch_concurrency' }, { text: '👥 Per Cycle', callback_data: 'set_wallets_per_cycle' }],
                    [{ text: '💵 Fund Amt', callback_ 'set_fund_amount' }, { text: '🗑️ Clear', callback_ 'pool_clear' }],
                    [{ text: '« Back', callback_ 'back_to_main' }]
                ]
            }
        }
    );
...
function showHelp(chatId) {
    bot.sendMessage(chatId,
        ...
        `• Balance checks prevent failed transactions`,
        {
            parse_mode: 'Markdown',
            reply_markup: {
                inline_keyboard: [[{ text: '« Back', callback_ 'back_to_main' }]]
            }
        }
    );
}
