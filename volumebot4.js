    ...
    try {
        ...
        for (const w of activeWhales) {
            ...
            try {
                ...
                logger.error(`Whale accumulate error: ${e.message}`);
            }
        }
...
bot.on('callback_query', async (callbackQuery) => {
    ...
    // Wallet Pool
        ...
        promptSetting(chatId, `💰 *Fund Wallet Pool*\n\nPool: \`${walletPool.size.toLocaleString()}\` wallets\nAmount: \`${STATE.fundAmountPerWallet}\` SOL each\n*Est. Cost: \`${estCost}\` SOL*\n\nReply \`YES\` to confirm:`, async (val) => {
            ...
            await withRpcFallback(async (connection) => {
                bot.sendMessage(chatId, `💰 Funding ${walletPool.size.toLocaleString()} wallets (depth: ${STATE.makerFundingChainDepth})...`);
                const targets = Array.from(walletPool.wallets.values());
                await fundWallets(connection, masterKeypair, targets, STATE.fundAmountPerWallet, chatId);
            });
        });
    }
    ...
    // Settings Handlers with validation
    ...
    else if (action === 'toggle_web_funding') { STATE.useWebFunding = !STATE.useWebFunding; showStealthSettings(chatId); }
    else if (action === 'toggle_stealth_level') { STATE.fundingStealthLevel = STATE.fundingStealthLevel === 2 ? 1 : 2; showStealthSettings(chatId); }
        ...
        promptSetting(chatId, `Reply with *Max Buy Amount* in SOL (0.0005 - 10):`, (val) => {
            try { STATE.maxBuyAmount = validateNumber(val, 0.0005, 10, "Max Buy"); bot.sendMessage(chatId, `✅ Max Buy: \`${STATE.maxBuyAmount}\` SOL`); } catch(e) { bot.sendMessage(chatId, `❌ ${e.message}`); }
            showBasicSettings(chatId);
        });
    }
